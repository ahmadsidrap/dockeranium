...
export default function SearchInput() {
    ...
    try {
      ...
      const filteredNetworks = data.filter((network: Network) => {
        ...
        const hasNetworkConfig =
          network.ipam &&
          Array.isArray(network.ipam) &&
          network.ipam.some(
            (config) => config && (config.Subnet || config.Gateway)
          );
  ...
  const handleBulkDeleteClick = () => {
    const networksToDelete = filteredNetworks.filter(
      (net) => selectedNetworks.has(net.id) && !net.inUse
    );
    ...
    try {
      ...
      for (const network of networksToDelete) {
        ...
        if (!response.ok) {
          const data = await response.json();
          throw new Error(
            data.error || `Failed to delete network ${network.name}`
          );
        }
      }
      ...
      setError(
        err instanceof Error ? err.message : "Failed to delete networks"
      );
    } finally {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
            ...
            <p className="text-gray-600 mb-4">
              Are you sure you want to delete these {networksToDelete.length}{" "}
              networks?
            </p>
              ...
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  <input
                    type="checkbox"
                    checked={
                      selectedNetworks.size === filteredNetworks.length &&
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredNetworks.map((network) => (
                <tr key={network.id} className="hover:bg-gray-50">
                  ...
                  `}
                      title={
                        network.inUse ? "Network is in use" : "Delete network"
                      }
                    >
                      Delete
                    </button>
